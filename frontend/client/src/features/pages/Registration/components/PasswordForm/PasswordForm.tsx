...
export const PasswordForm = ({ onPreviousStep, onNextStep, form, rules }: RegistrationFormProps) => {
    ...
    return <Styled.Stack>
        <Styled.PasswordInput size="md" required label="PASSWORD" placeholder="Enter your password" {...form.getInputProps("password")} />
        <RequirementsList rules={rules.password} />

        <Styled.PasswordInput size="md" required label="CONFIRM PASSWORD" placeholder="Confirm your password" {...form.getInputProps("confirmPassword")} />
